...
def measure(llm, prompt, target, k):
    ...
    pred = logits[len(p_ids) - 1: len(ids) - 1].argmax(axis=1)
    match = (pred == np.asarray(t_ids)).astype(int)
...
def main():
    ...
    if a.prompts:
        ...
        prompts = (SAMPLE_PROMPTS * ((a.n // len(SAMPLE_PROMPTS)) + 1))[: a.n]
        print("NOTE: using built-in sample prompts; pass --prompts with your real jobs for a domain number.")
    ...
    total = matched = 0; runs_all = []; per_prompt = []
    t0 = time.time()
    ...
    print(f"\nacceptance {acc:.3f} over {total} tokens; tokens per sweep at k={a.k}: "
